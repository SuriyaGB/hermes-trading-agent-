...
def save_financial_state(state: Dict[str, Any]):
    """Syncs the ledger to disk (Fix 7: Recon writing)"""
...
async def fetch_ibkr_data() -> Dict[str, Any]:
    ...
    try:
        ...
        clientId = int(os.getenv("IBKR_CLIENT_ID", 9))

        print(f"[LOG] Connecting to IBKR at {host}:{port}...")
        await ib.connectAsync(host, port, clientId=clientId)
        print("[LOG] Eye Opening: Requesting Delayed Data (Type 3)")
        ib.reqMarketDataType(3) 
        
        # 1. Fetch AAPL Stock Price (with yfinance fallback)
        print("[LOG] Audit: Fetching AAPL Spot Price...")
        ...
        if data["price_seen"] == 0.0:
            print("[LOG] WARNING: IBKR Price missing. Attempting yfinance fallback...")
            ticker_yf = yf.Ticker('AAPL')
            data["price_seen"] = round(ticker_yf.fast_info.last_price, 2)
        
        print(f"[LOG] Result: AAPL Spot = ${data['price_seen']}")

        # 2. Position Audit
        print("[LOG] Audit: Scanning Portfolio for active AAPL contracts...")
        ...
        # If we have a target strike, prioritize it. Otherwise, take the first AAPL option.
        pos = None
        if target_strike:
            print(f"[LOG] Precision Targeting: Looking for managed strike ${target_strike}...")
            selected = [p for p in opt_pos if p.contract.strike == target_strike]
            if selected: pos = selected[0]
        
        if pos is None and opt_pos:
            pos = opt_pos[0]
            print(f"[LOG] Auto-Targeting: Found unmanaged position {pos.contract.localSymbol}")
        ...
        if pos:
            ...
            print(f"[LOG] Target Locked: Analyzing {contract.localSymbol} (Strike: ${contract.strike})")
            
            if not contract.exchange: contract.exchange = 'SMART'
            await ib.qualifyContractsAsync(contract)
            [opt_ticker] = await ib.reqTickersAsync(contract)
            
            # --- DELTA RESCUE (LOCAL MATH FALLBACK) ---
            mkt_price = opt_ticker.marketPrice()
            data["delta_source"] = "ibkr_realtime" # Default
            
            if opt_ticker.modelGreeks and opt_ticker.modelGreeks.delta is not None:
                print("[LOG] Decision: Using IBKR Server-Side Greeks.")
                data["delta_seen"] = round(abs(opt_ticker.modelGreeks.delta), 4)
            else:
                # Local Math Engine Trigger
                print("[LOG] WARNING: IBKR Greeks Missing. Activating Local Math Engine...")
                try:
                    ...
                    if S > 0 and mkt_price > 0:
                        ...
                        data["delta_source"] = f"local_math_engine (r={round(r*100,2)}%)"
                        print(f"[LOG] Result: Local Math calculated Delta = {data['delta_seen']} (r={round(r*100,2)}%)")
                except Exception as ex:
                    print(f"[LOG] ERROR: Local math failed: {str(ex)}")

            # --- EMERGENCY BLACKOUT FALLBACK ---
            if data["delta_seen"] is None:
                print("[LOG] CRITICAL: Total Data Blackout. Applying Emergency Default 0.20.")
                data["delta_seen"] = 0.20 # Conservative Default (User Request)
            ...
            if math.isfinite(mkt_price) and pos.avgCost != 0 and mkt_price > 0:
                ...
                print(f"[LOG] P&L Audit: Current Profit/Loss = {data['pnl_pct']}%")

    except Exception as e:
        print(f"[LOG] SYSTEM ERROR: {str(e)}")
        data["error"] = f"IBKR Error: {str(e)}"
    finally:
        ib.disconnect()
    return data
...
def get_ibkr_analysis() -> Dict[str, Any]:
    ...
    # --- RECONCILIATION LOGIC (Fix 7) ---
    # Detect Assignment Event: IBKR Reality vs Stale Ledger
    if final_data["account_status"] == "SHARES_ASSIGNED" and state.get("current_phase") == "CSP_ACTIVE":
        ...
        if strike and premium is not None:
            ...
            final_data["cost_basis"] = adj_basis # Update pulse context immediately
