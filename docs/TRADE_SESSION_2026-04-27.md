...
| Time (IST) | AAPL Spot | Put Mkt Value | Bot P&L % | Actual P&L % | Phase |
|---|---|---|---|---|---|
| Session Open | ~$267.49 | ~$0.02 | ~99.57% | Not yet closed | `CSP_ACTIVE` |
| Pulse Triggered | $268.25 | ~$0.02 | 99.59% | Not yet closed | → `CLOSE_FOR_PROFIT` |
| After Close | N/A | **Filled @ $2.00** | 99.59% (theoretical) | **57.8% (real)** | `CASH_ONLY` |

> **IMPORTANT — Two Different P&L Numbers:**
> - **99.59%** = what the bot calculated using the live market price ($0.02). This is theoretical.
> - **57.8%** = the REAL profit based on the actual fill price ($2.00). This is the correct number.
> - **Why the gap?** The option's last market price was $0.02, but the actual bid/ask spread was wide
>   (e.g. bid $0.01 / ask $3.90). The limit order was placed at the mid-point = ~$2.00.
>   You cannot fill at $0.02 in reality. The $2.00 fill is normal for deep OTM paper trading exits.

> **IMPORTANT — The contract was NOT held to May 15 expiry.**
> You exited on April 27 with **17 days still remaining**. You bought back your obligation early.
> The May 15 expiry date on the contract still exists in the market — you simply stepped out of it.
> This is the correct and preferred approach in Wheel strategy (close early, redeploy capital).
...
```
Real Profit % = (Profit per share / Premium received) × 100
              = ($2.74 / $4.74) × 100
              = 57.8%

NOT 99.59%. That was theoretical (based on $0.02 live market price).
Actual fill was $2.00 due to wide bid/ask spread on deep OTM option.
```

| % Figure | Source | Meaning |
|---|---|---|
| **99.59%** | Bot's live calculation using $0.02 market price | Theoretical — unachievable due to spread |
| **57.8%** | Actual fill: $4.74 sold, $2.00 bought back | **Real profit. Correct number.** |

> **Why is 57.8% still a good result?**
> Wheel strategy common targets: 50% = good, 75% = great, 90%+ = excellent.
> 57.8% is above the 50% standard target.
> Given earnings were only 4 days away, closing at 57.8% was the correct risk-managed decision.
> Holding longer for 90% would have exposed the position to AAPL earnings gap risk.
